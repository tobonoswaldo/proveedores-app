...
//var_dump($_SERVER['REQUEST_METHOD'], $_POST, $_GET); exit;
...
if (!$oc) { safe_status(404); exit('OC no encontrada'); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accion === 'asociar') {
        ...
        try {
            ...
            // Construye la URL de forma segura
            $url = 'dashboard.php?' . http_build_query([
              'page'  => 'ordennes'       // no concatenes a mano; http_build_query se encarga
            ]);
            // Redirección por JS (no depende de headers)
            echo '<script>window.location.replace(' . json_encode($url) . ');</script>';
            echo '<noscript><meta http-equiv="refresh" content="0;url=' . htmlspecialchars($url, ENT_QUOTES) . '"></noscript>';
    ...
    if ($accion === 'quitar') {
        ...
        try {
            ...
            // Construye la URL de forma segura
            $url = 'dashboard.php?' . http_build_query([
              'page'  => 'ordenes'       // no concatenes a mano; http_build_query se encarga
            ]);
            // Redirección por JS (no depende de headers)
            echo '<script>window.location.replace(' . json_encode($url) . ');</script>';
            echo '<noscript><meta http-equiv="refresh" content="0;url=' . htmlspecialchars($url, ENT_QUOTES) . '"></noscript>';
...
<div class="wrap">
  ...
  <div class="grid-2 gap">
    ...
    <div class="card">
      ...
      <form method="post" 
                      onsubmit="return confirm('¿Quitar las facturas seleccionadas de esta OC(<?= (int)$oc['id']?>)?');">
        <input type="hidden" name="oc_id" value="<?= (int)$oc['id'] ?>">
